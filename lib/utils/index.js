 ...
 *   `port` - Defaults to `config.publicPort` or `config.ssl.port`, depending if SSL is enabled.
...
exports.buildUrl = function buildUrl(config, options) {
  options = options || {};
  options.protocol = options.protocol || config('protocol');
  options.hostname = options.hostname || config('host');
  options.port = options.port || 'https' == options.protocol ? config('ssl').port : config('publicPort');
  log("-------- BUILDING URL WITH: %s - %s - %s", options.protocol, options.hostname, options.port);
